            ...
            for sgn in side_signs:
                ...
                if cat == "BUILDING":
                    ...
                    for _ in range(cluster_count):
                        ...
                        proto = protos[rng.randrange(0, len(protos))]
                        ...
                        if hit is None:
                            continue
                        loc, _ = hit
                        _instance_collection("RWB_BUILDING_Instance", proto, target, loc, rot, sc)
                    ...
                    if hit is None:
                        continue
                    loc, _ = hit
                    _instance_collection(f"RWB_{cat}_Instance", proto, target, loc, rot, sc)
                    grid.insert(pos_xy.x, pos_xy.y)
                    if cat != "GRASS":
                        global_grid.insert(pos_xy.x, pos_xy.y)
                    count += 1
